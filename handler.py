...
MAX_INPUT_DURATION_SECONDS = get_int_env("MAX_INPUT_DURATION_SECONDS", 15)
...
def input_duration_limit(input_data):
    raw_value = input_data.get("max_input_duration_seconds", MAX_INPUT_DURATION_SECONDS)
    try:
        return max(0, int(raw_value))
    except (TypeError, ValueError):
        return MAX_INPUT_DURATION_SECONDS


def limit_video_duration(input_path, output_path, max_seconds):
    if max_seconds <= 0:
        return None
    ...
    try:
        ...
        return {
            "error": f"Video trim timed out after {FFMPEG_TIMEOUT_SECONDS} seconds",
            "stdout": tail_text(e.stdout, 1000),
            "stderr": tail_text(e.stderr, 1000),
        }

    if result.returncode != 0:
        return {"error": f"Video trim failed: {result.stderr}"}

    if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
        return {"error": "Trimmed video file was not generated or is empty"}
...
def run_latentsync(job):
    ...
    max_input_duration_seconds = input_duration_limit(input_data)
    ...
    with tempfile.TemporaryDirectory(dir="/tmp") as tmpdir:
        ...
        # 긴 입력은 서버리스 타임아웃/OOM을 피하기 위해 앞부분만 처리
        inference_video_path = video_path
        if max_input_duration_seconds > 0:
            limited_video_path = os.path.join(tmpdir, "input_video_limited.mp4")
            error = limit_video_duration(video_path, limited_video_path, max_input_duration_seconds)
            if error:
                return error
            inference_video_path = limited_video_path
        ...
        try:
            r = subprocess.run([
                "ffmpeg", "-i", audio_path,
                *(
                    ["-t", str(max_input_duration_seconds)]
                    if max_input_duration_seconds > 0
                    else []
                ),
                "-ar", "16000", "-ac", "1",
